...
export const ScrambleWords = () => {
  ...
  const [guess, setGuess] = useState(""); //input de adivinanza ---> valor de juego
  ...
  const renewState = () => {
    //new states (vars locales)
    const newArray = words.slice(1);
    const newShuffleArray = shuffleArray(newArray);
    const newCurrentWord = newShuffleArray[0];
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (guess === currentWord) {
      setPoints((prev) => prev + 1); //sumo un punto
      renewState();
      confetti({
        particleCount: 100,
        spread: 120,
  ...
  const handlePlayAgain = () => {
    const resetWords = shuffleArray(GAME_WORDS);
    const resetCurrentWord = resetWords[0];
    const resetScrambleWord = scrambleWord(resetCurrentWord);
    setWords(resetWords);
    setCurrentWord(resetCurrentWord);
    setScrambledWord(resetScrambleWord);
    setPoints(0);
    setErrorCounter(0);
    setSkipCounter(0);
    setIsGameOver(false);
